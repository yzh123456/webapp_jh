...
var Map = (function() {
	...
	var StaticGPS = {
		...
		drawAr: function(obj, x, y, angle) { //父元素obj, x:left, y:top ,角度
			...
			oImg.src = getBasePath() + "/qrcode/dynamic/images/ar.png";
	...
	var ManageClick = {
		...
		startPoint: function(aSvgPos) {    //点击从这里出发
			...
			if (Map.StaticGPS.bIsClickStart) {
				...
				if (!Map.StaticGPS.bEnd) {
					...
					$("#js_footer").hide();
					//出现导航中
					if ( !$("#sFloor_btn").html() || $("#sFloor_btn").html()=="" ) {
		...
		cancleGps: function() {    //点击确定(放弃导航)  ManageClick.cancleGps()
			...
			tmpPoint=null;tmp_ty=0;tmp_tx=0;//取消导航，重置起点信息
			//导航div影藏
			var oUpper = document.querySelector('.upper');    
			oUpper.style.display = 'none';
			...
			$("#sFloor_btn").html('');
            $("#end_btn").css("background-image","url(./images/js_img/navigation_hint_end.png)");///导航中部分，重置终点图标
			...
			document.querySelector('#imgTwo').style.display = 'none';
			document.querySelector('#line').innerHTML = '';
			Map.StaticGPS.hideTrajectory();//清除路径
			//动态点影藏
		...
		defaultBtn: function() {
			...
			if (!Map.StaticGPS.bClickMap) {
				...
				if (ManageClick.bClickBtn) {
					Map.HandleNode.setStyle(oDefault, {'background-image': "url(\'images/lou-01.png\')", 'color': '#fff'});    //蓝色带箭头
					oPtionBtn.style.display = 'block';
				} else {
					Map.HandleNode.setStyle(oDefault, {'background-image': "url(\'images/lou-01.png\')", 'color': '#fff'});    //灰色
			...
			success: function(str) {
		        ...
		        if (data != null) {
		          if (data.code == "success") {
		            ...
		            floorList = data.data.floorList;
		            floorId = data.data.floorList[0].id;
		            makeFloor(data.data.floorList);
		            getPointXY(start_point_code,"start");
		            loadRegion();
		          }
		        }
		     },
