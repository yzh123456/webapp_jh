...
let invoice;// 充电桩显示状态
let totalFee; // 应缴金额
class QueryCostDetail extends Component{
    constructor(...args){
        super(...args);//调用父级的构造器
        this.state = {
            stat:true
        ...
        JHTAJAX({
            ...
            success:function(data){
                ...
                if (data.dataitems != null && data.dataitems.length > 0) {
                    //金额显示只保留2位小数，如果为0则显示0  （返回的金额单位为分）
                    let ds;
                    ...
                    if(data.message){
                        el.payReason.children[1].innerText = data.message;
                    }
                    if (data.dataitems[0].attributes.tradestatus == 0) {
                        ...
                        el.tradeStatus.children[1].innerText= '订单关闭';
                        el.tradeStatus.id = "500";
                        el.refund.className="hide";
                    } else if (data.dataitems[0].attributes.tradestatus == 600) {
                        el.tradeStatus.children[1].innerText= '无效订单';
                    ...
                    // 在app运行环境下，且是代扣支付不成功的条件
                    if(that.jht.working()=="APP"
                        && data.dataitems[0].attributes.tradestatus != 500
                        && data.dataitems[0].attributes.tradestatus != 0
                        && data.dataitems[0].attributes.tradestatus != 100
                        && data.dataitems[0].attributes.ordertype == "DK" ){
                        el.goToPay.className="go_to_pay";
                    }else {
                        el.goToPay.className="go_to_pay hide";
                    }
                    // 发票
                    // if(invoice == 1||invoice == 2){
                    //     el.invoice.className="invoice";
                    //     el.voiceParent.className="";
                    // }
                    ...
                    if(data.dataitems[0].attributes.is_refund == "1" && Number(Number(get_unix_time(data.dataitems[0].attributes.createtime))+604800+"000") >= new Date().getTime()){
                        if(data.dataitems[0].attributes.ordertype == "CDP"||data.dataitems[0].attributes.ordertype == "SR"){
                            ...
                            // 发票
                           // if(invoice == 1||invoice == 2){
                           //      el.invoice.className="invoiceRight";
                           //      el.refund.className="refundLeft";
                           //  }else{
                           //      el.refund.className="refund";
                           //  }
                            el.refund.className="refund";
    ...
    handleRefundClick(event){
        console.log(this.refs.refund.className);
        let attr = {};
        ...
        if(that.stateObj.check == "checked" && jht.urlParams().parkId ){
            ...
            let orderno = [];
            document.querySelectorAll(".list_div .checked").forEach((check, i)=> {
                orderno.push(check.id);
            });
    ...
    handleInvoiceClick(event){
        ...
        let attr = {};
        attr.USER_ID = window.USERINFO.USER.USER_ID;
        attr.SOURCE = this.jht.working() == "APP" ? "app":"web";
        attr.ORDER_NO = query.text;
        getInvoice(attr);
    }
    ...
    handleSureClick(event){
        ...
        AppBridge.baseData(function (data) { //请求native获取usertId
            ...
            let query = that.props.location.query;
            let obj = {
                attributes:{
                    ORDER_NO:query.text,
                    USER_ID:userId
    ...
    //去缴费，与app交互
    goToPay(){
        if(this.jht.working()=="APP"){
            AppBridge.forJS({
                serviceId:"PAYDKORDER",
                params:{
                    orderNo: orderNo,
                    totalFee:totalFee
                }
    ...
    render(){
        ...
        let poptip = {"display":"none"};
        let voiceParent = {"color": "#ffffff","margin": "0.30rem","fontSize": "0.36rem","height": "0.84rem"};
        return (
            <div className="first_class">
                ...
                <div className="ul_parent" ref="ulParent">
                    <ul className="ul">
                        ...
                        <li className="show" ref="bankType">
                            <span>支付方式</span>
                            <span className="second_class"></span>
                        </li>
                        ...
                        <li className="li_last" ref="payReason">
                            <span>订单审核结果</span>
                            <span className="second_class"></span>
                        </li>
                    </ul>
                </div>
                <div style={voiceParent} className="hide" ref="voiceParent">
                    <div id="refund" className="hide"  onClick={this.handleRefundClick.bind(this)} ref="refund">申请退款</div>
                    <div id="invoice" className="hide" onClick={this.handleInvoiceClick.bind(this)} ref="invoice">申领发票</div>
                </div>
                ...
                <div className="mask_new hide" ref="maskNew" onClick={ this.handleMaskClick.bind(this) }>
                </div>
                <footer className="footer">
                    <div className="go_to_pay hide" id="" ref="goToPay" onClick={this.goToPay.bind(this)}>去支付</div>
                    <div className="tell">若您对上述交易有疑问</div>
...
// }
export { QueryCostDetail };
// export { QueryCostDetail,makeCode };